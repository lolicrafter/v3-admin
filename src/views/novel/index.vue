    ...
    <div class="search-box">
      <el-form :inline="true" :model="data.searchForm" class="demo-form-inline">
        <el-form-item label="小说名称">
          <el-input v-model="data.searchForm.name" placeholder="请输入小说名称" />
        </el-form-item>
...
import myUpload from '@/views/novel/components/upload.vue'
const data = reactive({
  searchForm: {
    _id: '',
    name: ''
...
const handleWatch = (item:{ name: string }) => {
  router.push({
    path: '/novel/catalog',
    query: {
      name: item.name
